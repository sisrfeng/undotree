...
"=================================================
    ...
    " 'Diff' sign definitions.
        ...
        "\ a 'normal' one and
        "\ one  that is
        "\ used if the very end of the buffer has been deleted
        "(in which case the deleted text is actually bejond the end of the current buffer version and
        "\ therefore it is not possible to place a sign on the exact line
        "\ because  it doesn't exist.
        "\ Instead,
            "\ a 'special' delete sign is placed on the (existing)
    ...
    " Id to use for all signs.
        ...
        " which is a shame because  it needs to be!
        " Note that all signs are placed with the same Id
        " - as long as we keep a count of how many we have placed
        "   (so we can remove them all again),
        " this is ok
...
"
    ...
    " Helper function, do action in target window, and then update itself.
        ...
        fun! s:undotree.ActionDiffToggle() abort
            echom  '我在call t:diffpanel.Toggle()'
    ...
    fun! s:undotree.Update() abort
        if !self.IsVisible()  | return  | en

        " do nothing if we're in the undotree or diff panel
        if exists('b:isUndotreeBuffer')  | return  |  en
        ...
        \ || (mode() != 'n')
            ...
            \ && self.targetid == w:undotree_id
                call s:log("undotree.Update() invalid buffer NOupdate")
                return
            en
            " This is not a valid buffer, could be help or something.
            let emptybuf = 1
        ...
        el
            ...
            if self.targetBufnr == bufnr('%')
                ...
                if self.rawtree == newrawtree  | return  | en
...
"diff panel
    ...
    fun! s:diffpanel.Update(seq,targetBufnr,targetid) abort
        ...
        if !self.diffexecutable  | return  | en
        ...
        el
            ...
            el
                ...
                set eventignore=all
                    ...
                    let diffresult = split(
                                        \ system(g:undotree_DiffCommand . ' ' . tempfile1 . ' ' . tempfile2) ,
                                        \ "\n",
                                        \ )
                    call s:log("变量diffresult是: " . string(diffresult))
                ...
                "\ echom "a:targetBufnr . '_' . a:seq 是: "   a:targetBufnr . '_' . a:seq
                "\ echom "self.cache 是: "   self.cache
                "\ echom "diffresult 是: "   diffresult
            en
        en
    ...
    fun! s:diffpanel.ParseDiff(diffresult, targetBufnr) abort
        ...
        " If 'a:diffresult' is empty
            "  there are no new signs to place.
        " However,
            "\ if we are at the very first sequence,
                " we need to ensure any old signs are removed.
            " otherwise
                " signs get left
        ...
        for line in a:diffresult
            ...
            if !empty(matchnum)
                ...
                if matchwhat ==# 'd'
                    if g:undotree_HighlightChangedWithSign
                        ...
                        " the line number we have is always 1 less than the line
                        " we  need to place the sign at,
                            " hence '+ 1'
                        " However,
                            " if the very end of the buffer has been deleted
                            " then this is not possible (because  that bit of the buffer no longer exists),
                            " so we place a 'special' version of the 'delete'  sign on
                            " what is the last available line

                        exe 'sign place ' . s:signId
                                \  . ' line=' . (
                                                \ (lineNr < l:lastLine)
                                                  \ ? lineNr + 1
                                                  \ : l:lastLine
            ...
            if empty(matchtext)  | continue  | en
            ...
            if g:undotree_HighlightChangedText
                if matchtext != ' '
                    ...
                    call add(
                        ...
                        \ matchadd(  (matchwhat ==# 'a'
                                        \ ? g:undotree_HighlightSyntaxAdd
                                        \ : g:undotree_HighlightSyntaxChange
                                    \ ),
                                    \ matchtext
                                \ ),
                       \ )
